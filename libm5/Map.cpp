...
bool cBmp5::Load(int mapid)
{
	...
	Free();
	_cache = cMapReader::getInstance()->getCache(mapid);
...
cMap::cMap()
{
	...
	_isThread = false; // threadsCount > 0;
	if (_isThread) {
		for (int i = threadsCount; i > 0; --i) {
			threads.emplace_back(std::thread(&cMap::renderMap, this));
		}
	}
}
...
void cMap::renderMap()
{
	while (g_StateType != STATE_EXT) {
		std::unique_lock<std::mutex> lock(mutex);
		while (!pMapRender) condition.wait(lock);
		pMapRender->step(cocos2d::Vec2(m_xCenter, m_yCenter), true);
		pMapRender = nullptr;
	}
}
...
BOOL cMap::Show(BYTE* pPic, RECT rect)
{
	...
	if (_isThread){
		std::unique_lock<std::mutex> lock(mutex);
		pMapRender = cMapRender::getInstance();
