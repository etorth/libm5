...
#if defined(ccc_date)
	m_TPcDate[28].SetString(std::string("加点:") + std::to_string(MyTest::getInstance()->per加点));
#endif
	for (int i = 0; i < 29; i++)
	{
...
#if defined(ccc_date)
	m_TPcDate[28].SetXY(m_xPos + 160, m_yPos + 224);  //加点值
#else
...
#if defined(ccc_date)
			m_TPetProPerty[16].SetString(MyTest::getInstance()->per加点);
#endif
			for (int i = 0; i < 19; i++)
			{
...
void cInterfaceDialog::Talk(sTalkList* talk)
{
	...
	pObj = g_pMainState->FindObjByName(name);
	if (pObj)
	{
...
void cInterfaceDialog::InitQuestion()
{
	...
	ostringstream oss;
#if !defined(ccc_date)
	oss << g_MediaPath<<"任务//题库//";
	oss << "0.txt";
#else
	oss << g_MediaPath <<"题库.txt";
#endif
	File.open(oss.str());
	int num;
	File >> num;
