...
void cTaskBaotu::Init()
{
	...
	ostringstream oss;
#if !defined(ccc_date)
	oss << g_MediaPath << "任务//打宝图//百家姓.txt";
#else
	oss << g_MediaPath << "强盗名字.txt";
#endif
	File.open(oss.str());
	int num;
	File >> num;
	...
	oss.str("");
#if !defined(ccc_date)
	oss << g_MediaPath << "任务//打宝图//地点.txt";
#else
	oss << g_MediaPath << "强盗地点.txt";
#endif
	File.open(oss.str());
	File >> num;
	string str;
...
void cTaskZhuaGui::Init()
{
	...
	ostringstream oss;
#if !defined(ccc_date)
	oss << g_MediaPath << "任务//抓鬼//百家姓.txt";
#else
	oss << g_MediaPath << "抓鬼名字.txt";
#endif
	File.open(oss.str());
	if (!File)ERRBOX;
	int num;
	...
	oss.str("");
#if !defined(ccc_date)
	oss << g_MediaPath << "任务//抓鬼//地点.txt";
#else
	oss << g_MediaPath << "抓鬼地点.txt";
#endif
	File.open(oss.str());
	if (!File)ERRBOX;
	File >> num;
...
void cTaskSM::InitShiWei()
{
	...
	ostringstream oss;
#if !defined(ccc_date)
	oss << g_MediaPath << "任务\\师门\\师门示威.txt";
#else
	oss << g_MediaPath << "师门\\师门示威.txt";
#endif
	File.open(oss.str());
	if (!File)ERRBOX;
	string name;
...
BOOL cTaskFB::Start(int id)
{
	...
	ostringstream oss;
#if !defined(ccc_date)
	oss <<g_MediaPath<<"任务//副本//" << id << ".txt";
#else
	oss << g_MediaPath << "副本//" << id << ".txt";
#endif
	File.open(oss.str());
	if (!File)ERRBOX;
	int num;
...
BOOL cDialogTrigger::sDialogTrigger::Process(BOOL skip /*= FALSE*/)
{
	...
	if (!skip)
	{
		if (m_Nowtext < m_DialogList.size())
		{
			...
			if (2 == m_Mode) // 2代表普通对话
			{
				auto name = m_DialogList[m_Nowtext].name;
				if (name == "篮子" || name == "主角") name = g_pCharacter->m_PCData.m_Name;
				g_pMainState->m_Channel.AddNewString(m_DialogList[m_Nowtext].context,name, 1);
			}
			m_Nowtext += 1;
			return TRUE;
		}
