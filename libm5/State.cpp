...
BOOL CMainState::Frame(void* pData)
{
	if (!isInit)
	{
...
#else
		{
			...
			int index = -1, size = savs.size();
			forv (savs, k)
			{
				if (savs.at(k).find("无") == std::string::npos && savs.at(k).find("最新") != std::string::npos)
				{
...
static const std::vector<ulong> getWeaponDies(int i);
void CMainState::LoadList()
{
	...
	if (!cGame::getInstance()->isIter()) {
		m_pCharacterDataList[eJuSe逍遥生].m_Pos[1].m_stand = m_pCharacterDataList[eJuSe逍遥生].m_Pos[0].m_stand;
		return;
	}

	auto func = [this](int r, ulong u){
		auto& pos = m_pCharacterDataList[r].m_Pos;
		pos[0].m_dead = pos[1].m_dead = u;
	};
	func(eJuSe飞燕女, 0x7B45E2A6);
	func(eJuSe狐美人, 0x0EBE00EA);
	func(eJuSe神天兵, 0x6252396E);
	func(eJuSe虎头怪, 0x67C54CFB);
	func(eJuSe逍遥生, 0x6A3DDE8E);
	func(eJuSe龙太子, 0x93FC6108);
	func(eJuSe巨魔王, 0xAFF25120);
	func(eJuSe舞天姬, 0xB5643809);
	func(eJuSe玄彩蛾, 0x5977EF1B);


	auto func2 = [this](int r, int wtype, int idx) {
		auto& wlist = m_ItemManage.m_pWeaponList2[wtype];
		int wid = (wlist[0][0].Master == r) ? 0 : 1;
		if (idx < 0) {
			for (int i = 0; i < 11; ++i) {
				wlist[i][wid].m_stand = getWeaponDies(idx)[i];
			}
		} else {
			for (int i = 0; i < 11; ++i) {
				wlist[i][wid].m_dead = getWeaponDies(idx)[i];
			}
		}
	};
	func2(eJuSe虎头怪, eWeaponType斧, 0);
	func2(eJuSe巨魔王, eWeaponType斧, 1);
	func2(eJuSe逍遥生, eWeaponType扇, -1);
	func2(eJuSe逍遥生, eWeaponType扇, 2);
	func2(eJuSe龙太子, eWeaponType扇, 3);
	func2(eJuSe飞燕女, eWeaponType双剑, 4);
	func2(eJuSe狐美人, eWeaponType爪, 5);
	func2(eJuSe神天兵, eWeaponType枪, 6);
	func2(eJuSe玄彩蛾, eWeaponType飘带, 7);
	func2(eJuSe舞天姬, eWeaponType飘带, 8);

	{
		auto& pos = m_pCharacterDataList[eJuSe羽灵神].m_Pos;
		pos[2].m_majic = pos[0].m_majic;
	}
	{
		auto& pos = m_pCharacterDataList[eJuSe杀破狼].m_Pos;
		pos[1].m_walk = pos[2].m_walk;
	}

	auto func3 = [this](int r, ulong uid) {
		m_pCharacterDataList[r].m_ColorID = uid;
	};
	func3(eJuSe逍遥生, 0x001);
	func3(eJuSe剑侠客, 0x002);
	func3(eJuSe飞燕女, 0x003);
	func3(eJuSe英女侠, 0x004);
	func3(eJuSe巨魔王, 0x005);
	func3(eJuSe虎头怪, 0x006);
	func3(eJuSe狐美人, 0x007);
	func3(eJuSe骨精灵, 0x008);
	func3(eJuSe神天兵, 0x009);
	func3(eJuSe龙太子, 0x010);
	func3(eJuSe舞天姬, 0x011);
	func3(eJuSe玄彩蛾, 0x012);
	func3(eJuSe巫蛮儿, 0x203);
	func3(eJuSe杀破狼, 0x205);
	func3(eJuSe羽灵神, 0x209);
}

const std::vector<ulong> getWeaponDies(int i) {
	switch (i) {
		// 01 斧
	case 0: return { 0x2208A659, 0x1DDBB1C6, 0x6F6E119A, 0xEC47773C, 0x4CBC0CC5, 0xE74758DC, 0xC9417A1A, 0x77ACF46A, 0x436E551F, 0x9953B8A3, 0x32F0DC6C };
	case 1: return { 0xEBC7D5D6, 0x0690D9A1, 0x5342D0DD, 0x64D25284, 0xCA673C2C, 0x9FB8EBFE, 0xB49ADB36, 0xCDE7B933, 0x91B7053D, 0x9C68E3FE, 0x1401A457 };
			// 23 扇
	case 2: return { 0xFA5C9E94, 0x1E1C8C68, 0xC4A8BE71, 0x65DAA4AF, 0x47B23554, 0x0E73A34E, 0xE3FD6ACE, 0x4B84B4Bc, 0x6EEB6938, 0xBDF71787, 0x00000000 };
	case 3: return { 0x0779CD74, 0xBA525A8F, 0x257FDF2A, 0xD7AF7F94, 0x8C0689D3, 0xB02F871D, 0x0B9C4539, 0x55DEC235, 0x8C0689D3, 0x8D249D24, 0xE41424F5 };
			// 4  双剑
	case 4: return { 0x919BF5DC, 0x1A7F20D6, 0xACC10E65, 0x05514FE8, 0xCC6E99E4, 0x36E7DDA5, 0x5AB67B17, 0x60445950, 0x3DC61885, 0x46384768, 0xD731D412 };
			// 5  刺
	case 5: return { 0x8554BA42, 0x2E7BEC5D, 0xF1C3EB05, 0xF32CE8B9, 0x06338B65, 0x3017B6E1, 0x09EB6B04, 0xF63172D0, 0x535E76D0, 0xC4DA3341, 0xD40EE4A7 };
			// 6  枪
	case 6: return { 0x8C0A81B3, 0xDD1CA29D, 0x0003261F, 0x935502D2, 0x5788173C, 0x4229E403, 0x26056CA0, 0x695191D6, 0x80BDB343, 0x39CEFA42, 0xB9B8F39E };
			// 78 飘带 
	case 7: return { 0xD6BCEC1B, 0x65040B9D, 0xBCCF18C6, 0x456F97AD, 0x98FD03B4, 0xD370F823, 0x73E31106, 0xBD9D313D, 0xB7D84088, 0x2A5B6A13, 0xB4A8DCD0, };
	case 8: return { 0x6B5C6C40, 0x12C0084E, 0x017EC87E, 0xA18D027D, 0x6903FD70, 0x116DF6C7, 0x5B54BEEA, 0x42063DCA, 0xCFF08DA4, 0xCF089AF3, 0xB12EFC5A, };
	}
	// 逍遥生扇子
	return { 0x5A34B119, 0xD33792DB, 0x0032EADD, 0xC5D0FFA5, 0x836BD211, 0xC9D1E4D9, 0x2EF87FD7, 0x9D8FF3C2, 0xD940F97E, 0x9297DD11, 0xE481F72B, };
}
...
#else
			m_Challange.OnOff();
			// if (m_Challange.m_NeedShow) m_Tags.Add("注意：战斗死亡按正常处理！");
#endif
			return TRUE;
		}
...
//	m_hFont[5] = EasyCreateFont(12,  "汉仪魏碑简");
	...
	m_FNumRed.Load(0x30F737D8);
	m_FNumGreen.Load(0x3CF8F9FE);
...
//	m_Back.LoadBMP(g_strMediaPath + "数据/开始界面/" + cc::toString(rand() % 13) + ".bmp");
	...
	Move(g_320 - 320/*m_Back.GetWidth() / 2*/, g_240 - 240/*m_Back.GetHeight() / 2*/);
...
// 	m_Back.SetXY(g_320 - m_Back.GetWidth() / 2, g_240 - m_Back.GetHeight() / 2);
	...
	m_Title.SetY(g_240 - 240/*m_Back.GetHeight() / 2*/ + 200);
