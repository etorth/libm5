...
#if defined(ccc_date)
	ifstream File0;
	string s = g_MediaPath + "_境外导标旗.txt";
	File0.open(s);
	if (!File0)ERRBOX(5);
#endif
	for (int i = 0; i < 5; i++)
...
BOOL cInterfaceChannel::Init()
{
	...
	ostringstream oss;
#if !defined(ccc_date)
	oss << g_MediaPath << "数据//表情//表情.txt";
#else
	oss << g_MediaPath << "表情.txt";
#endif
	File.open(oss.str());
...
BOOL cTags::Init()
{
	...
	_tagMax = std::min(c10, (g_480 - 160) / 25);
	m_Text.m_hFont = 0;
...
void cTags::Process()
{
	if (0 == m_TagNum) {
		m_Sleeptime = 0;
		for (int i = 0; i < c10; i++) m_tags[i].m_time = 0;
		return;
	}
	if (m_Sleeptime > 0)
	{
...
void cInterfaceInstructions::init()
{
	...
	_titles.push_back("设置");
	_titles.push_back("快捷键");
	_titles.push_back("手册");
	...
	sChild btn(&g_pMainState->m_Button2, "<--", _backw / 4 - 24, 12);
	btn.onClick = [this]()
	{
		if (_page > 0)
		{
	...
	sChild btn2(&g_pMainState->m_Button2, "-->", _backw * 3 / 4 - 24, 12);
	btn2.onClick = [this]()
	{
...
#if !defined(ccc_date)
	str += "\n内容与原作者#y子犯#Y一致,不更新功能,只优化体验及修复,存档通用";
#else
	str += "\n#Y日期版#Y，主打 擂台、比武、剧情挑战,建议先上手#Y梦幻群侠传5#Y";
#endif
	str += "\n\n";
	cc::setQ(str);

	str += " 只在群里发布";
	str += "\n     不存在其他途径(任何网站论坛贴吧网盘某宝某鱼夕夕...)";
	str += "\n     #Y谨防感冒#Y受骗";

	int y1 = y + _ftexts[0].m_Height + 80 + 80;
	{
		auto& b = g_bFillScreen;
		sChild c(&g_pMainState->m_Button4, b ? "全屏模式" : "窗口模式", x, y1 - i3);
	...
	{
		sChild c("当前音乐状态 ", x, y1);
		c.tag = cTagCfg;
		addChild(c);
		cText5* t = (cText5*)c.ptr;
		auto& b16 = g_pMainState->m_GlobalButton[16];
		sChild c2(&g_pMainState->m_Button2, b16 ? "关" : "开", x + t->m_Width, y1 - i3);
	...
	{
		sChild c("当前音效状态 ", x, y1);
		c.tag = cTagCfg;
		addChild(c);
		cText5* t = (cText5*)c.ptr;
		auto& b17 = g_pMainState->m_GlobalButton[17];
		sChild c2(&g_pMainState->m_Button2, b17 ? "关" : "开", x + t->m_Width, y1 - i3);
		cText5* t2 = (cText5*)c2.ptr2;
	...
	{
		sChild c("重开游戏生效 ", x, y1);
		c.tag = cTagCfg;
		cText5* t = (cText5*)c.ptr;
		addChild(c);
		sChild c2(&g_pMainState->m_Button2, "配置", x + t->m_Width, y1 - i3);
		cText5* t2 = (cText5*)c2.ptr2;
		c2.onClick = [](){
			std::string path = __argv[0];
			path = cc::replace(path, "/", "\\");
			...
			path += cc::replace(game->getPath(), "/", "\\"); 
			ShellExecute(NULL, "open", (path + game->getExe()).c_str(), 0, path.c_str(), SW_SHOWNORMAL);
		};
		c2.tag = cTagCfg;
		addChild(c2);
		y1 += iy;
...
#if !defined(ccc_date)
	{
		sChild c("重开新档生效 ", x, y1);
		c.tag = cTagCfg;
		cText5* t = (cText5*)c.ptr;
		addChild(c);
		sChild c2(&g_pMainState->m_Button2, "主角", x + t->m_Width, y1 - i3);
		cText5* t2 = (cText5*)c2.ptr2;
		c2.onClick = []() {
			std::string path = __argv[0];
			path = cc::replace(path, "/", "\\");
			...
			auto game = cGame::getInstance();
			path += cc::replace(game->getPath(), "/", "\\");
			ShellExecute(NULL, "open", (path + "hero.bat").c_str(), 0, path.c_str(), SW_SHOWNORMAL);
		};
		c2.tag = cTagCfg;
		addChild(c2);
		y1 += iy;
...
#endif
	str += "\n#Y平时#Y:";
	str += "\nA.界面  E.包裹  G.给予  X.交易  T.队伍  O/P.宝宝  Q.任务  R.副本  C.快捷  F.好友  S.系统";
	str += "\nW 主角属性面板";
	str += "\nF9 仙店灵铺";
#if defined(ccc_date)
	str += "\nF10 切换模型";
...
#else
	str = "1.升级自动满技满修";
	str += "\n2.#Y重新邀请#Y队友会#Y还原#Y属性，故#Y不建议#Y给队友宝宝喂元宵";
#endif
	add(2, str);
