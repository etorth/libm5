...
const std::string& cMapReader::getFileNameWithoutRoot(int mapid)
{
	const auto& it = _searchs.find(mapid);
	if (it != _searchs.end()){
		return it->second;
	}
	if (cGame::getInstance()->isMaper() && mapid == 1001 && rand() % 100 < 50) {
		mapid = 4001;
	}
	std::string name = toString("%d.map", mapid);
	std::vector<std::string> fs = { "jd", "ori", "it" };
	if (cGame::getInstance()->isMaper()) {
		std::swap(fs[0], fs[2]);
	}
	string path;
	for (const auto& f : fs) {
		path = g_DLC + "scene." + f + "/" + name;
		static std::ifstream ifile;
		ifile.open(path, std::ios::binary);
		if (ifile.is_open()) {
			ifile.close();
			_searchs.insert(std::make_pair(mapid, path));
...
{
	sMapCache c;
	const auto& filenameWithoutRoot = getFileNameWithoutRoot(mapid);
